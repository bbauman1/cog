...
**SHELL VARIABLE SUBSTITUTION:** When passing shell environment variables to `lim ios type`, use **double quotes**, not single quotes. Single quotes pass the literal text (e.g., `'${DEVIN_API_KEY}'` types the literal string `${DEVIN_API_KEY}`). Use `lim ios type "${DEVIN_API_KEY}"` to resolve the variable first.
...
## Testing Audio/Mic Features

### Simulator Limitations

- **`SpeechTranscriber.isAvailable` returns `false`** on the Limrun simulator. This means the mic button (gated by `speechService.isAvailable`) will NOT appear in CreateSessionView or SessionDetailView by default.
- **`lim ios app-log` and `lim ios syslog` do NOT capture `os.Logger` structured logs.** They only show stderr/system messages. To verify `os.Logger` output, use a real device with Xcode Console or `log stream --predicate 'subsystem == "com.cogfordevin.ios"'`.
- **The simulator has a virtual audio device** — `AVCaptureDevice.default(for: .audio)` returns non-nil and `isInputAvailable` returns `true`. However, the virtual device reports **invalid format** (sampleRate=0, channelCount=0).

### Testing Mic Button Crash Scenarios

To test mic/audio code paths that are normally hidden because `SpeechTranscriber.isAvailable` is `false` on the simulator:

1. **Temporarily modify `isAvailable`** in `SpeechTranscriptionService.swift` to return `true`:
   ```swift
   var isAvailable: Bool { true }
   ```
2. **Bypass the internal `SpeechTranscriber.isAvailable` guard** inside `startTranscription()` (comment it out) — otherwise the guard fires before reaching the audio hardware guards.
3. **Bypass the `SpeechTranscriber.supportedLocale()` guard** — this also depends on the Speech framework and will return nil on the simulator. Use `let locale = Locale.current` as a dummy replacement.
4. **Rebuild and install**: `lim xcode build . --scheme Cog --configuration Debug --ios`
5. After testing, **revert all temporary changes** and rebuild to confirm clean compile.

**Expected behavior with bypasses:** The code will flow through permission check → AVCaptureDevice check (passes — simulator has virtual device) → audio session config → isInputAvailable (passes) → format validation guard catches invalid format → shows "Audio input format is not supported" error. The app should NOT crash.

### Mic Button Accessibility
- **AXLabel:** `"Start dictation"` (when not transcribing) / `"Stop dictation"` (when transcribing)
- **AXUniqueId:** `"mic"`
- Located in the Prompt section of CreateSessionView and the message input bar of SessionDetailView

## App Logs

```bash
# Tail recent app logs (stderr/system messages only)
lim ios app-log com.cogfordevin.ios -n 30

# Stream app logs continuously
lim ios app-log com.cogfordevin.ios -f

# Stream system logs
lim ios syslog -n 50
```

Note: These do NOT capture `os.Logger` structured logging. For structured logs, use a real device.
...
- `SpeechTranscriber.isAvailable` returns `false` on the Limrun simulator — mic button is hidden by default (see "Testing Audio/Mic Features" section for workaround)
- `os.Logger` structured logs are not capturable via `lim ios app-log` or `lim ios syslog` — only stderr/system messages appear
- The simulator's virtual audio device reports invalid format (sampleRate=0, channelCount=0) — format validation guards will fire
- `annotate_recording` tool does NOT work with `lim ios record` — it only works with desktop recordings. For iOS simulator recordings, use screenshots at key moments as evidence instead
